...
9. [HR Policy Q&A (RAG)](#9-hr-policy-qa-rag)
...
2. Update the connection string in `database.py` with your SQL Server instance name:
   ...
   ```
3. Tables are created automatically on first run via `models.Base.metadata.create_all()`.
...
```bash
# Install and pull a lightweight local model
ollama pull phi3
```
Add HR policy documents (PDF or DOCX) to the `hr_policies/` folder. Then trigger ingestion (admin only) via a `POST` request to `/hr/ingest`.
...
| **PUT** | `/employees/{id}` | Admin Only | Update an employee |
| **DELETE** | `/employees/{id}` | Admin Only | Delete an employee |
| **POST** | `/hr/ingest` | Admin Only | Re-index HR policy documents into the vector store |
...
3. Protected routes require an `Authorization: Bearer <token>` header; the token is verified on every request via a FastAPI dependency.
